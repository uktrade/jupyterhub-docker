''' Initialises /etc/rstudio/connections/ with database connections

Copies environment variables in the form DATABASE_DSN__.... to R files which
rstudio runs as database connections
'''
...
for name, dsn in env['DATABASE_DSN'].items():
    ...
    with open('/etc/rstudio/connections/{name}.R'.format(name=name), 'w') as f:
        f.write('library(DBI)\n')
        f.write('con <- dbConnect(RPostgres::Postgres(), user="{user}", password="{password}", host="{host}", port="{port}", dbname="{dbname}")\n'.format(
            user=user,
            password=password,
            host=host,
            port=port,
